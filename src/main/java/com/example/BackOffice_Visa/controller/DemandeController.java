...
public class DemandeController {
        ...
        private static final Integer TYPE_DEMANDE_TRANSFERT_VISA = 2;
        ...
        public String showCreateDemandeForm(
                ...
                if (step == 3) {
                        ...
                        if (wizard.getIdTypeVisa() != null) {
                                piecesSpecifiques = pieceJustificativeService
                ...
                if (step == 4) {
                        ...
                        model.addAttribute("selectedTypeVisa",
                                        typeVisaService.findById(wizard.getIdTypeVisa()).orElse(null));
        ...
        public String finalizeDemande(
                ...
                if (wizard.getIdTypeVisa() == null || wizard.getIdTypeDemande() == null
                                || wizard.getDateDemande() == null) {
                        redirectAttributes.addFlashAttribute("errorMessage",
                                        "Veuillez compléter l'étape 3 avant de finaliser.");
                        return "redirect:/demandes/nouveau?step=3";
                }
                ...
                boolean transfertVisa = TYPE_DEMANDE_TRANSFERT_VISA.equals(wizard.getIdTypeDemande());
                boolean duplicataCarte = TYPE_DEMANDE_DUPLICATA_CARTE.equals(wizard.getIdTypeDemande());

                Demande savedDemande;

                if (transfertVisa || duplicataCarte) {
                        boolean reconstitutionMode = false;
                        boolean carteResidentAntecedenteAbsente = false;

                        if (transfertVisa) {
                                reconstitutionMode = visaTransformableService
                                                .findByNumeroReference(wizard.getNumeroReferenceVisaTransformable())
                                                .isEmpty();
                        } else {
                                carteResidentAntecedenteAbsente = carteResidentService
                                                .findLatestByNumeroPasseport(wizard.getNumeroPasseport())
                                                .isEmpty();
                                reconstitutionMode = carteResidentAntecedenteAbsente;
                        }

                        Demandeur demandeur = demandeurService
                                        .findByIdentity(wizard.getNom(), wizard.getPrenom(), wizard.getDateNaissance())
                                        .orElseGet(() -> demandeurService.save(buildDemandeurFromWizard(wizard)));

                        Passeport passeport = passeportService.findByNumeroPasseport(wizard.getNumeroPasseport())
                                        .orElseGet(() -> {
                                                Passeport nouveauPasseport = new Passeport();
                                                nouveauPasseport.setDemandeur(demandeur);
                                                nouveauPasseport.setNumeroPasseport(wizard.getNumeroPasseport());
                                                nouveauPasseport.setDateDelivrance(wizard.getDateDelivrance());
                                                nouveauPasseport.setDateExpiration(wizard.getDateExpiration());
                                                nouveauPasseport.setPaysDelivrance(wizard.getPaysDelivrance());
                                                RefStatutPasseport statutPasseport = refStatutPasseportService
                                                                .findById(1)
                                                                .orElseThrow(() -> new IllegalArgumentException(
                                                                                "Statut passeport #1 introuvable"));
                                                nouveauPasseport.setStatutActuel(statutPasseport);
                                                return passeportService.save(nouveauPasseport);
                                        });

                        Optional<VisaTransformable> visaTransformableExistant = visaTransformableService
                                        .findByNumeroReference(wizard.getNumeroReferenceVisaTransformable());

                        VisaTransformable visaTransformable = visaTransformableExistant.orElseGet(() -> {
                                VisaTransformable nouveauVisaTransformable = new VisaTransformable();
                                nouveauVisaTransformable.setDemandeur(demandeur);
                                nouveauVisaTransformable.setPasseport(passeport);
                                nouveauVisaTransformable.setDateDebut(wizard.getDateDebutVisaTransformable());
                                nouveauVisaTransformable.setDateExpiration(wizard.getDateExpirationVisaTransformable());
                                nouveauVisaTransformable
                                                .setNumeroReference(wizard.getNumeroReferenceVisaTransformable());
                                return visaTransformableService.save(nouveauVisaTransformable);
                        });
                        ...
                        RefStatutDemande statutDemande = refStatutDemandeService
                                        .findById(reconstitutionMode ? STATUT_DEMANDE_VALIDEE : STATUT_DEMANDE_CREEE)
                                        .orElseThrow(() -> new IllegalArgumentException("Statut demande introuvable"));
                        demande.setStatut(statutDemande);
                        demande.setDateDemande(wizard.getDateDemande());
                        if (reconstitutionMode) {
                                demande.setDateTraitement(LocalDate.now());
                        }
                        savedDemande = demandeService.save(demande);

                        if (reconstitutionMode && transfertVisa) {
                                Visa visaFinal = new Visa();
                                visaFinal.setDemande(savedDemande);
                                visaFinal.setPasseport(passeport);
                                visaFinal.setDateDebut(wizard.getDateDebutVisaTransformable());
                                visaFinal.setDateFin(wizard.getDateExpirationVisaTransformable());
                                visaFinal.setReference("VISA-" + savedDemande.getId());
                                visaService.save(visaFinal);
                        }

                        if (reconstitutionMode && duplicataCarte) {
                                if (carteResidentAntecedenteAbsente) {
                                        CarteResident carteReconstituee = new CarteResident();
                                        carteReconstituee.setDemande(savedDemande);
                                        carteReconstituee.setPasseport(passeport);
                                        carteReconstituee.setDateDebut(wizard.getDateDebutVisaTransformable());
                                        carteReconstituee.setDateFin(wizard.getDateExpirationVisaTransformable());
                                        carteReconstituee.setReference("RECONSTITUEE-" + savedDemande.getId());
                                        carteResidentService.save(carteReconstituee);
                                }

                                CarteResident duplicataFinal = new CarteResident();
                                duplicataFinal.setDemande(savedDemande);
                                duplicataFinal.setPasseport(passeport);
                                duplicataFinal.setDateDebut(wizard.getDateDebutVisaTransformable());
                                duplicataFinal.setDateFin(wizard.getDateExpirationVisaTransformable());
                                duplicataFinal.setReference("DUPLICATA-" + savedDemande.getId());
                                carteResidentService.save(duplicataFinal);
                        }
                        ...
                        if (reconstitutionMode) {
                                redirectAttributes.addFlashAttribute("successMessage",
                                                "Demande créée et validée automatiquement (ID: " + savedDemande.getId()
                                                                + ")");
                        } else {
                                redirectAttributes.addFlashAttribute("successMessage",
                ...
                demande.setTypeDemande(typeDemandeService.findById(wizard.getIdTypeDemande())
                                .orElseThrow(() -> new IllegalArgumentException("Type demande introuvable")));
                RefStatutDemande statutDemande = refStatutDemandeService.findById(STATUT_DEMANDE_CREEE)
        ...
        private Demandeur buildDemandeurFromWizard(DemandeWizardData wizard) {
                Demandeur demandeur = new Demandeur();
                demandeur.setNom(wizard.getNom());
                demandeur.setPrenom(wizard.getPrenom());
                demandeur.setDateNaissance(wizard.getDateNaissance());
